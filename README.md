...
## Dataset
We derive SciHTC from papers available in the ACM digital library. The category information of the papers in our dataset were defined based on the category hierarchy tree named 'CCS' created by ACM. Specifically, each paper is assigned to a sub-branch of the hierarchy tree. In addition to the category information, the title, abstract and author-specified keywords are also specified for each paper in our dataset. For a more detailed description of our dataset construction process and its properties, we refer the reader to our dataset description [paper](paper_url). 

### Dataset Size
We split our dataset in a 80:10:10 ratio for train, test and development sets, respectively. The number of papers in each split are as follows:
  * Train: 148,928 
...
### Dataset Access
We make the ACM paper IDs and the binarized labels (whether each topic/label is relevant to a certain paper) publicly available [here](some_url). Please email us at msadat3@uic.edu for getting access to the full dataset.
